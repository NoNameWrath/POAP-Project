...
export function Claim({ enabled, claimToken, merkleProof, onMinted }: {
  enabled: boolean
  claimToken: string
  merkleProof?: string[]
  ...
  async function handleClaim() {
    ...
    try {
      ...
      if (!check.ok) throw new Error('Claim ticket refused')

      const umi = makeUmi()
      umi.use(createSignerFromWalletAdapter(wallet.adapter as any))

      await mintV2(umi, {
        candyMachine: publicKey(CANDY_MACHINE_ID),
        collectionMint: publicKey(COLLECTION_MINT),
        guards: { allowList: merkleProof ? { merkleProof } : undefined },
      }).sendAndConfirm(umi)

      // Optional: Ask server to resolve mint (placeholder returns null)
  ...
  return (
    <Card>
      <div className="flex items-center justify-between">
        <h3 className="font-semibold flex items-center gap-2"><Medal className="w-5 h-5" /> Claim NFT</h3>
      ...
      <div className="mt-4">
        ...
        >
          {busy ? (<span className="inline-flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin"/> Minting…</span>) : 'Claim My POAP'}
        </button>
        {err && (<div className="mt-4 text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg p-3 flex items-center gap-2"><AlertTriangle className="w-4 h-4" /> {err}</div>)}
      </div>
    </Card>
  )
}
